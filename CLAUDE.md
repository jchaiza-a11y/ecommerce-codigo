...
> `AGENTS.md` en la raíz lo regenera automáticamente `next dev`/`next build` con
> advertencias de breaking changes de esta versión de Next.js. No se edita a mano;
> se lee antes de escribir código que toque APIs del framework.
...
8. Rutas y endpoints de admin protegidos en `proxy.ts` (renombrado desde `middleware.ts` en Next.js 16) **y** con verificación por código de permiso en el handler (`requirePermission('products.create')`). Comparar nombres de rol en el código (`role === 'admin'`) es hallazgo bloqueante.
